...
* Bevat de auth-fixes in `node-lox-ws-api` voor moderne firmware (zie `CHANGES-NL.md` voor details):
  - HMAC-key wordt nu als raw bytes gebruikt i.p.v. via UTF-8 decode (Hash.js, AES-256-CBC.js, Token-Enc.js)
  - Token-request gebruikt `jdev/sys/getjwt/...` op MS ≥ 10.2 (Token-Enc.js + API.js versie-detectie)
  - Command-chain regex matcht zowel `gettoken` als `getjwt`
