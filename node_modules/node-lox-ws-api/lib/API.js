...
var API = function (host, username, password, reconnect, security, keepalive) {
    // FIX HTTPS: een 'https://'-prefix op host schakelt TLS in (https:// voor de
    // HTTP-calls, wss:// voor de websocket). Certificaatvalidatie staat dan uit:
    // het Loxone-certificaat is uitgegeven op de cloud-DNS-naam en matcht nooit
    // een lokaal IP. Zonder prefix blijft het gedrag exact zoals voorheen.
    this._secure = /^https:\/\//i.test(host);
    this._host = host.replace(/^https?:\/\//i, '');
...
API.prototype.connect = function() {
    var connection = new Connection(this._host, this._keepalive, this._secure);
...
API.prototype.perform_version_check = function(connection) {
    var that = this;
    // FIX HTTPS: kies transport op basis van this._secure. Bij TLS wordt het
    // servercertificaat niet gevalideerd: het Miniserver-cert is uitgegeven op
    // {snr}.dns.loxonecloud.com en matcht per definitie niet het lokale IP.
    var transport = this._secure ? https : http;
    var scheme = this._secure ? 'https://' : 'http://';
    var options = this._secure ? { rejectUnauthorized: false } : {};
    transport.get(scheme+this._host+'/jdev/cfg/api', options, (res) => {
        res.on('data', (chunk) => {
            if (res.statusCode === 200) {
                var json = JSON.parse(chunk);
                var api = JSON.parse(json.LL.value.replace(/'/g,'"'));
                var version = api.version.split(".");
                // FIX: bewaar miniserver-versie als int-array, Token-Enc gebruikt dit
                // om te kiezen tussen 'jdev/sys/gettoken' (legacy) en 'jdev/sys/getjwt' (≥10.2).
