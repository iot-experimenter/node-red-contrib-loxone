...
TokenEnc.prototype._get_public_key = function() {
    ...
    // FIX HTTPS: volg de secure-vlag van de API (zie API.js). Cert-validatie is
    // uit omdat het Miniserver-cert op de cloud-DNS-naam staat, niet op het IP.
    var secure = !!(this._api && this._api._secure);
    var transport = secure ? https : http;
    var scheme = secure ? 'https://' : 'http://';
    var options = secure ? { rejectUnauthorized: false } : {};
    transport.get(scheme+this._host+'/jdev/sys/getPublicKey', options, (res) => {
        res.on('data', (chunk) => {
...
TokenEnc.prototype._register_getkey2_response = function() {
    ...
    this._api.command_chain.push({
        ...
        'callback': function(loxone_message) {
            ...
            // FIX 1: HMAC-key moet raw bytes uit hex zijn, niet UTF-8 reinterpretation.
            // Oude code (new Buffer(value, 'hex').toString('utf8')) verliest non-UTF8 bytes
            // → corrupt HMAC → 401 op alle moderne miniservers.
            // Vergelijk PyLoxone: HMAC(bytes.fromhex(key_hex), ...).
            var key = Buffer.from(loxone_message.value.key, 'hex');
            ...
            // FIX 2: dynamisch getjwt (≥10.2) of gettoken (legacy) kiezen.
            // De API-versie wordt door API.js in this._api._ms_version geplaatst.
...
TokenEnc.prototype._register_gettoken_response = function() {
    var that = this;
    this._api.command_chain.push({
        // FIX: matcht zowel gettoken (legacy) als getjwt (≥10.2).
...
TokenEnc.prototype._register_getkey_for_refresh_response = function() {
    ...
    this._api.command_chain.push({
        ...
        'callback': function(loxone_message) {
            // FIX: HMAC-key moet raw bytes uit hex zijn (zie getkey2-fix hierboven).
            var key = Buffer.from(loxone_message.value, 'hex');
            var hmac = crypto.createHmac(that._sha, key);
