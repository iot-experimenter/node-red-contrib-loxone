...
// NB: deze legacy-auth (Miniserver <= v8) ondersteunt bewust geen HTTPS —
// API.js schakelt op firmware >= 9 altijd naar Token-Enc, die het wel kan.
...
AES.prototype._register_keyexchange_response = function() {
    ...
    this._api.command_chain.push({
        ...
        'callback': function(loxone_message) {
            // FIX: HMAC-key moet raw bytes uit hex zijn, niet via UTF-8 herinterpreteerd.
            // Vergelijk PyLoxone: HMAC(bytes.fromhex(key_hex), ...).
            var key = Buffer.from(that._decipher(loxone_message.value), 'hex');
