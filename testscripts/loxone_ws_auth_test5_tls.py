"""
Loxone Gen2 authentication test â€” PyLoxone-master 1:1 replica + diagnostiek.
...
DIAGNOSTIEK (alleen lokaal gelogd, NIET extra over de draad):
  ...
  - EÃ©n poging, geen retry-loop, hard stop bij 401 (om IP-block te vermijden).
"""
...
# CONFIG
# ============================================================
HOST = "<your-miniserver-ip>"
PORT = 443  # TLS-variant: firmware >= 17 sluit poort 80
...
TOKEN_PERMISSION = int(_os_perm.environ.get("TOKEN_PERM", "2"))
...
def _redact(s: str, keep: int = 6) -> str:
    if len(s) <= keep * 2:
        return s
    return f"{s[:keep]}â€¦{s[-keep:]} (len={len(s)})"
...
async def http_phase(session: aiohttp.ClientSession) -> tuple[list[int], str, str]:
    ...
    async with session.get(
        url, auth=auth, timeout=aiohttp.ClientTimeout(total=TIMEOUT), ssl=False
    ) as resp:
...
async def ws_recv_ll(ws, label: str) -> dict[str, Any]:
    ...
    log.debug(f"[WS:{label}] wachten op header (8 bytes)â€¦")
    ...
    log.debug(f"[WS:{label}] wachten op payloadâ€¦")
...
async def run() -> None:
    ...
    # ---- WebSocket
    ws_url = f"wss://{HOST}:{PORT}/ws/rfc6455"
    _banner(f"WebSocket connect â†’ {ws_url}")
    ...
    # TLS zonder certvalidatie: Miniserver-cert staat op {snr}.dns.loxonecloud.com,
    # matcht dus nooit het lokale IP.
    import ssl as _ssl
    ...
    async with wslib.connect(ws_url, **ws_kwargs) as ws:
        ...
        if decoded_salt is not None:
            log.info(f"[WS]    salt (HEXâ†’UTF8 decoded) = {decoded_salt!r}  (len={len(decoded_salt)})")
        else:
        ...
        if decoded_salt is not None:
            log.info("â”€â”€â”€â”€ DIAGNOSTIEK: hash met decoded salt (NIET verzonden) â”€â”€â”€â”€")
        ...
        import os as _os
        if _os.environ.get("NO_JWT"):
            log.info("[DIAG] NO_JWT gezet â€” stop vÃ³Ã³r getjwt (geen loginpoging).")
            return
        ...
        if code != 200:
            ...
            log.error("Hard-stop om IP-block te voorkomen â€” geen retry.")
            if decoded_salt is not None:
